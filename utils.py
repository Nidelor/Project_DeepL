...
def vector_to_scenes(scenes_vector):
    ...
    for example in range((scenes_vector.shape[0])): #Batch
        for x in range((scenes_vector.shape[2])): #Time point
...
def vector_to_obj(obj):
    ...
    categories = f.read().split("\n")
    for example in range(obj.shape[0]): #Batch
        for x in range(obj.shape[2]): #Time Point
